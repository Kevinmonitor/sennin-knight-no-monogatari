...
func _ready() -> void:
	if MainGame.get_singleton().obtainedSlash:
		queue_free()
	
func _on_body_entered(body: Node2D) -> void:
	print("cocacola boots with the ladle")
	if MainGame.get_singleton().obtainedSlash:
		queue_free()
		MainGame.get_singleton().obtainedSlash = true
		MainGame.get_singleton().playerPath._gainSlash()
	else:
